# Contributing to Hetzner Cloud Plugin
...
With Go installed, you can already `go get` the Hetzner Cloud plugin and `make dev` in
...
3. One of Hetzner Cloud plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Hetzner Cloud plugin
project. After following the steps in "Setting up Go to work on the Hetzner Cloud plugin":
...
Please only apply the minimal vendor changes to get your PR to work. The Hetzner Cloud plugin
...
If the Hetzner Cloud Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
