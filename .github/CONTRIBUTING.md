...
runtime in order to build packer with the Hcloud plugin.
...
## Setting up Hcloud plugin for dev

With Go installed, you can already `go get` the Hcloud plugin and `make dev` in
...
1. Download the Hcloud plugin source (and its dependencies) by running
...
2. When working on the Hcloud plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-hcloud`
...
3. Make your changes to the Hcloud plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
...
4. After building the Hcloud plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-hcloud/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Hcloud plugin](https://github.com/hashicorp/packer-plugin-hcloud#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-hcloud f-my-feature
```
...
3. One of Hcloud plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Hcloud plugin
project. After following the steps in "Setting up Go to work on the Hcloud plugin":
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-hcloud.git`
...
Please only apply the minimal vendor changes to get your PR to work. The Hcloud plugin
...
If the Hcloud Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
