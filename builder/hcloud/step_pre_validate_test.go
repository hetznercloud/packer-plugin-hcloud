...
func TestStepPreValidate(t *testing.T) {
	RunStepTestCases(t, []StepTestCase{
		...
		{
			Name: "server type is deprecated",
			Step: &stepPreValidate{
				SnapshotName: "dummy-snapshot",
			},
			SetupConfigFunc: func(c *Config) {
			...
			SetupStateFunc: func(sb multistep.StateBag) {
				sb.Put(StateUI, &packersdk.MockUi{})
			},
			WantRequests: []mockutil.Request{
				{
					Method: "GET", Path: "/server_types?name=cpx22",
					...
					"server_types": [{
						...
						"architecture": "x86",
						"deprecation": {
							"announced": "2098-01-01T00:00:00+00:00",
							"unavailable_after": "2099-01-01T00:00:00+00:00"
						}
					}]
				}`,
				},
			...
			WantStepAction: multistep.ActionContinue,
			WantStateFunc: func(t *testing.T, sb multistep.StateBag) {
				ui, ok := sb.Get(StateUI).(*packersdk.MockUi)
				assert.True(t, ok)
				assert.True(t, ui.ErrorCalled)
				assert.NotEmpty(t, ui.ErrorMessage)
				assert.Contains(t, ui.ErrorMessage, "cpx22")
			},
		},
		{
			Name: "server type is unavailable",
			Step: &stepPreValidate{
				SnapshotName: "dummy-snapshot",
			},
			SetupConfigFunc: func(c *Config) {
			...
			WantRequests: []mockutil.Request{
				{
					...
					"server_types": [{
						...
						"architecture": "x86",
						"deprecation": {
							"announced": "1998-01-01T00:00:00+00:00",
							"unavailable_after": "1999-01-01T00:00:00+00:00"
						}
					}]
				}`,
				},
			...
			WantStepAction: multistep.ActionHalt,
			WantStateFunc: func(t *testing.T, sb multistep.StateBag) {
				err, ok := sb.Get(StateError).(error)
				assert.True(t, ok)
				assert.Error(t, err)
				assert.Contains(t, err.Error(), "cpx22")
			},
		},
		{
			Name: "upgrade server type is deprecated",
			Step: &stepPreValidate{
				SnapshotName: "dummy-snapshot",
			},
			SetupConfigFunc: func(c *Config) {
				...
				},
				{
					...
					JSONRaw: `{
						"server_types": [{
							...
							"deprecation": {
								"announced": "1998-01-01T00:00:00+00:00",
								"unavailable_after": "2999-01-01T00:00:00+00:00"
							}
						}]
					}`,
				},
			...
			WantStateFunc: func(t *testing.T, sb multistep.StateBag) {
				...
				assert.True(t, ui.ErrorCalled)
				assert.NotEmpty(t, ui.ErrorMessage)
				assert.Contains(t, ui.ErrorMessage, "cpx32")
			},
		},
		{
			Name: "upgrade server type is unavailable",
			Step: &stepPreValidate{
				SnapshotName: "dummy-snapshot",
			},
			SetupConfigFunc: func(c *Config) {
			...
			WantStateFunc: func(t *testing.T, sb multistep.StateBag) {
				...
				assert.True(t, ok)
				assert.Error(t, err)
				assert.Contains(t, err.Error(), "cpx32")
			},
		},
	})
}
