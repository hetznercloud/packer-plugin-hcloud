...
- `network` (string) - Attach the server to a Network.

- `ip_address` (string) - Specify the Private IP Address which will be used if
  you attach the server to a Network. The `network` option has to be provided.

- `alias_ips` (array of strings) - Provide the Server with Alias IPs. 
  Only in conjunction with `network`.

- `connect_with_private_ip` (bool) - Enable to use the Private Network Interface
  to connect to the server. Depends on `network`.

- `subnet` (string) - If `network` is specified, a Subnet can be set to specify
  where the Server get attached to. The identificator is the IPRange - the format
  is CIDR "10.10.1.0/24"

- `max_snapshots` (int) - Snapshot Lifecycle Management - set the maximum of
  concurrent Snapshots with the same Label.
  So if you set it to `max_snapshots` = 2 a total of 2 Snapshots with the same
  Label can coexists, if you rerun the build, the oldest Snapshots get deleted.

- `skip_image_creation` (bool) - If set to true, no Snapshot will be created.
...
build {
  sources = ["source.hcloud.hetzner"]
}
``` 

HCL Format with Label Selector:
